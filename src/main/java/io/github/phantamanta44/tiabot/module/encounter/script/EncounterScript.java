...
public class EncounterScript {
	...
	private static final String PKG_IMPORT = "var pkgEncData = JavaImporter(Packages.io.github.phantamanta44.tiabot.module.encounter.data); with (pkgEncData) {%s}";
	...
	public static void execute(String script, IEventContext ctx, BattleContext bc, StatsDto stats) {
		...
		try {
			...
			con.evaluateString(scope, String.format(PKG_IMPORT, script), "<enct>", 1, null);
		} catch (Exception ex) {
